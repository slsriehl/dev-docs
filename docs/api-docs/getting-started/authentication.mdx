...
<Callout type="info">
  Legacy API accounts used HTTP basic authentication. They are no longer being created for any reason. We strongly recommend updating any existing apps or connections utilizing legacy API authentication to update to Oauth. For more information, see [API Accounts: Migrating from legacy to OAuth](/api-docs/getting-started/api-accounts#migrating-from-legacy-to-oauth).
</Callout>
...
### Client ID (If client-id is no longer required, why does this section exist?)
