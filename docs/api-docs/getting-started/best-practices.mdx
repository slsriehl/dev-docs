...
To keep data in your application up-to-date, [webhooks](/api-docs/getting-started/webhooks/about-webhooks) should be used in place of periodic API polling. Note: Periodic polling does not scale well as your store grows. Use an [OAuth API account](/api-docs/getting-started/api-accounts) to register and subscribe to webhook-enabled events relevant to your application.
...
### Avoid polling the Storefront Cart API (I'm wondering if we need to change this to just be `avoid polling` and incorperate the webhooks section here)
...
## Thread API requests (We should consider either flat-out removing this and saying do not thread, or putting a big disclaimer that its a bad idea with write traffic and can get you in trouble)
