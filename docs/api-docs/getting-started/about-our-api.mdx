...
HTTP response header names are case-insensitive; see the [HTTP specification on field names](https://www.rfc-editor.org/rfc/rfc9110.html#name-field-names) for more information. For example, your application may receive `x-rate-limit-requests-left` rather than `X-Rate-Limit-Requests-Left`, so plan your implementation accordingly. Most open source HTTP clients treat headers with the appropriate case insensitivity.
 ...
 -- Testing with v3 catalog/products endpoint on an employee sandbox: Headers returned
Content-type, Transfer-encoding, connection, vary, x-request-id, content-encoding, x-rate-limit (requests-left, time-reset-ms, requests-quota, time-window-ms)

-- testing v2 orders endpoint on employee sandbox: Headers returned
Content-type, transfer-encoding, connection, vary, x-request-id, last-modified (not in v3), content-encoding, x-bc-store-version (not in v3, not something we should be returning), x-bc-apilimit-remaining (not sure why we don't also provide this on v3), x-rate-limit headers, strict-transport-security
...
#### Response content type (What is this, does it even still exist? Probably good to get with the api-platform team to check)
...
## Support (I wonder if its a good idea to add a blurb here about ent/partners getting to contact support for api-related questions and have them answered)
