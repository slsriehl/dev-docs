...
| **202** | **Accepted** | For a request that resulted in a scheduled task being created to perform the actual request. | (do we ever return this status code?)
...
| **429** | **Too Many Requests** | When an OAuth client exceeds the [rate limit](/api-docs/getting-started/best-practices#best-practices_rate-limits) for API requests to a store. | (We may want to be a bit more verbose here. This can occur when we impose the limit on them, not just our system. Maybe something about this existing to protect the platform, and engineers may also impose artificial restrictions due to malfeasance. We also might want to drop a thing about "if we are throwing these at you, reevaluate what you are doing" here alongside the updated rate limiting docs)
...
| Code | Text | Purpose |
|:----:|:-----|:--------|
| **500** | **Internal Server Error** | When an error has occurred within the API. | (not sure I like this just being this vague. Merchants can and do cause 500s with their calls, typically going too fast with writes. Maybe including this in the "how to troubleshoot" area)
